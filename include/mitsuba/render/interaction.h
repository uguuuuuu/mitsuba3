...
struct Interaction {
    ...
    Interaction(Float t, Float time, const Wavelength &wavelengths,
                const Point3f &p, const Normal3f &n = 0.f, Float dA = 1.f)
        : t(t), time(time), wavelengths(wavelengths), p(p), n(n), dA(dA) { }
    ...
    DRJIT_STRUCT(Interaction, t, time, wavelengths, p, n, dA);
...
struct SurfaceInteraction : Interaction<Float_, Spectrum_> {
    ...
    MI_IMPORT_BASE(Interaction, t, time, wavelengths, p, n, dA, is_valid)
    ...
    explicit SurfaceInteraction(const PositionSample3f &ps,
        ...
        : Base(0.f, ps.time, wavelengths, ps.p, ps.n, ps.dA), uv(ps.uv),
    ...
    DRJIT_STRUCT(SurfaceInteraction, t, time, wavelengths, p, n, dA, shape, uv,
...
struct MediumInteraction : Interaction<Float_, Spectrum_> {
    ...
    DRJIT_STRUCT(MediumInteraction, t, time, wavelengths, p, n, dA, medium,
